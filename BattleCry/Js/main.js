...
var game = {
  ...
  renderPlayer : function() {
      ...
      loader.load("daes/avatar.dae", function ( collada ) {
        mesh = collada.scene;
        skin = collada.skins[ 0 ];

        mesh.scale.x = mesh.scale.y = mesh.scale.z = 300;// done like this to keep ratio
  ...
  renderLights : function() {
    ...
    scene.add( light );
    var light = new THREE.PointLight( 0xFFFFFF, 1, 1000 );
    light.position.set( 300, -900, 0 );
    scene.add( light );
  },

  render : function() {
    var delta = THREE.Clock.prototype.getDelta();
    if ( t > 1 ) t = 0;
    if ( skin ) { 
  ...
  moveMot : function(direction, f) {    
    if (game.moving == false) {
      ...
      function mot() {
        setTimeout(function(){
          if (f == '-') {mesh.position[direction] = mesh.position[direction] - 0.5; camera.position[direction] = camera.position[direction] - 0.5;
          } else {mesh.position[direction] = mesh.position[direction] + 0.5; camera.position[direction] = camera.position[direction] + 0.5;}
          needed--;
  ...
  toggleTree : function(id) {
    $('#tree').toggle();
    if (id == '0') return;
    for (i=0; i<armor.warrior.Swords.length; i++) {
      if (id == armor.warrior.Swords[i].id) {
        game.makeTree(armor.warrior.Swords[i].tree);
      }
    }
  },

  makeTree : function(edit) {
